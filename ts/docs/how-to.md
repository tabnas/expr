...
new Tabnas().use(jsonic).use(Expr, {
  op: {
    power: { infix: true, src: '^', left: 5100000, right: 5000000 },  // exponent tier, right-assoc
  },
}).parse('2^3^2')  // ['^', 2, ['^', 3, 2]]
```
...
Expr, {
  op: {
    bang:   { prefix: true,  src: '!', right: 5000000 },   // !x  (prefix tier)
    factor: { suffix: true,  src: '!', left:  6000000 },   // x!  (postfix tier)
  },
}
```
...
Expr, {
  op: {
    cond: { ternary: true, src: ['?', ':'], left: 1100000, right: 1200000 },
  },
}
```
