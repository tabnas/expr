...
- Go: `go/expr_test.go` — `runSpec(t, name, j)`, a `support.Runner{...}`
  over the file.
- Rust: `rs/tests/parity_test.rs` — `run_spec(name, parser)`, a
  `tabnas_support::Runner::new(...).file(path)` over the file.
...
a new file has to be wired into every runtime by hand. A fixture only one
...
- TypeScript is canonical. If the runtimes disagree, the TS behaviour is
  ...
  and `DIVERGENCE.md` record, which stay out of these shared fixtures.
