...
// convenience Parse() rebuilds the (expensive) expr grammar on every call
...
// MakeJsonic() instance.
//
// The check is machine-INDEPENDENT: it compares the no-options Parse()
// against instance reuse on the SAME machine in the SAME run, so a slow CI
// box cannot make it flaky (both sides scale together). There is deliberately
...
func TestParseReusesInstance(t *testing.T) {
	...
	// A rebuild-per-call Parse() is far slower here, so this catches the
	// regression without depending on absolute wall-clock speed.
	if conv > 4*reuse {
