...
// Package main builds the C-ABI shared library: libtabnasexpr.
//
// tabnas-clib-template: v4 (stamped by admin tasks/adopt-clib.sh;
// edit the template and re-stamp, not this file).
//
//	go build -buildmode=c-shared -o libtabnasexpr.so ./clib
//
// expr TEXT IN, VERDICT (AND VALUE) OUT. This is one of the
...
// tabnas_grammar builds a expr parser and returns a handle to it.
...
// tabnas_parse checks one input against the expr grammar.
//
//export tabnas_parse
