# libtabnasexpr — the expr parser as a C ABI

<!-- tabnas-clib-template: v5 — stamped by admin tasks/adopt-clib.sh;
     edit the template and re-stamp, not this file. -->

The expr format parser as a C shared library, so languages with no
tabnas port can parse and validate expr input. This is one of the
...
| Function | Returns |
|---|---|
| `tabnas_version()` | `{"ok":true,"lib":"libtabnasexpr","format":"expr","template":"v5"}` |
...
// link against libtabnasexpr; every call returns a JSON []u8 to free with
// c.tabnas_free.
```
